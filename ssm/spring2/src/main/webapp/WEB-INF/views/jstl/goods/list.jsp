...
<body>
	<div class="main">
		...
		<form action="<c:url value="/goods/deletes?pageNO=${pageNO}"/>"
			...
			<table border="1" width="100%" class="tab">
				...
				<c:forEach var="entity" items="${goods}">
					<tr>
						...
						<td>${entity.price}</td>
						<td><a
							href="<c:url value="/goods/"/>delete/${entity.id}?pageNO=${pageNO}"
							class="abtn">删除</a> <a
							href="<c:url value="/goods/"/>edit/${entity.id}" class="abtn">编辑</a>
							<a href="<c:url value="/goods/"/>upPicture/${entity.id}"
							class="abtn">上传</a></td>
					</tr>
				</c:forEach>
			</table>
			<div id="pager"></div>
			<p>
				<a href="add" class="abtn out">添加</a> <input type="submit"
					value="批量删除" class="btn out" />
			</p>
